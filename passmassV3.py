...
USAGE = '''passmass oldpassfile newpassfile [changpass-user]. . . .'''
COMMAND_PROMPT = '[$#] '
...
def change_password(child, user, chguser,oldpassword, newpassword):
    if chguser:
    	child.sendline('sudo passwd ' + chguser)
    else:
...
def main():
	...
	chguser=''
    	...
    	script1,file1,file2,chguser = sys.argv
	...
	newpassword = GeneratePassword()
        ...
        change_password(child, user, chguser,password, newpassword)
        child.expect(COMMAND_PROMPT)
        child.sendline('exit')
	g.write('%s,%s,%s,%s,%s\n' % (host,port,user,chguser,newpassword))
